...
foreach ($sizeMB in $sizesMB) {
    ...
    $commentHeader = "`n/* PADDING START`n"
    $commentFooter = "`nPADDING END */"
    ...
    $newContent = $originalContent + $paddedCommentBlock

    $outputFile = Join-Path $directory ("{0}_{1}MB_end{2}" -f $outputBaseName, $sizeMB, $extension)
